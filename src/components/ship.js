...
const ship = (shipName) => {
  ...
  /**
   * A function that calls other functions to hit the ship and always check if its sunk after the hit is taken.
   */
  const handleHit = () => {
    increamentHits();
    checkIfSunk();
  ...
  /**
   * A function that calls is handle hit to increase the amount of hits taken by the ship.
   */
  const isHit = () => { handleHit(); };
